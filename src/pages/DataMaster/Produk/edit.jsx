...
const EditProduk = () => {
    ...
    useEffect(() => {
        ...
        setisLoading(true);
        axios.get(`${Url}/products?kode=${id}`, {
            headers: {
                'Accept': 'application/json',
                'Authorization': `Bearer ${token}`
            }
    ...
    else if (data && piecesData?.length > 0 & categoryData?.length > 0 & gradesData?.length > 0 & typesData?.length > 0 & brandsData?.length > 0 & taxesData?.length > 0 && !isError)
